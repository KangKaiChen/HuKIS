...
class LayerLSTM(torch.nn.Module):
    def __init__(self, input_size, hidden_size, bias):
        super(LayerLSTM, self).__init__()
        self.LSTM = torch.nn.LSTMCell(input_size, hidden_size, bias)
...
class CustomNeuralNetwork(torch.nn.Module):
    def __init__(self):
        ...
        self.fc = torch.nn.Linear(4, 128 * 128 * 4)
...
class AdaCoFNet(torch.nn.Module):
    ...
    def forward(self, frames, F_kprime_to_k, F_n_to_k_s, F_k_to_n_s):
        ...
        #   disparity_features.append(self.spatialFeatureNetwork(disparity_image[i]))
        ...
        for idx, feat in enumerate(features):
            if self.args.all_backward > 0:
                ...
                # print(len(F_n_to_k_s), " second", idx)
                ref_frame_flow = torch.nn.ReplicationPad2d((W, W, H, H))(
                    F_n_to_k_s[idx]
                )  # 相鄰真到關進真的光劉
        ...
        if self.args.pooling_with_mask <= 0:
            ...
            if self.args.pooling_with_center_bias <= 0:
                ...
                print("global_summed_feature", global_summed_feature)
                global_summed_weights = torch.stack(tenWarpedMask, -1).sum(
                    -1
                )  # 0,1 判斷
                print("global_summed_weights", global_summed_feature)
        ...
        if self.args.no_pooling > 0 and self.args.single_decoder <= 0:
            ...
            if self.args.decoder_with_mask <= 0:
                ...
                for i in range(len(frames)):
                    ...
                    # temp = I_preds[i][0].cpu().numpy().transpose(1, 2, 0)
                    # temp[:, :, 0] = (temp[:, :, 0] - temp[:, :, 0].min()) / (
                    #     temp[:, :, 0].max() - temp[:, :, 0].min()
                    # )
                    # temp[:, :, 1] = (temp[:, :, 1] - temp[:, :, 1].min()) / (
                    #     temp[:, :, 1].max() - temp[:, :, 1].min()
                    # )
                    # temp[:, :, 2] = (temp[:, :, 2] - temp[:, :, 2].min()) / (
                    #     temp[:, :, 2].max() - temp[:, :, 2].min()
                    # )
                    # temp[:, :, 0] = (temp[:, :, 0] - 0.5) * 5 + 0.5
                    # temp[:, :, 1] = (temp[:, :, 1] - 0.5) * 5 + 0.5
                    # temp[:, :, 2] = (temp[:, :, 2] - 0.5) * 5 + 0.5
                    # print(temp[:, :, 0].max(), " ", temp[:, :, 0].min())
                    #      cv2.imwrite(f"I_preds_{i}.png", temp * 255)
                    imwrite(Cs[i], f"Cs{i}.png", normalize=True)
                    imwrite(I_preds[i], f"I_preds_{i}.png", normalize=True)
                    # cv2.imwrite(
                    #     f"Cs{i}.png",
                    # )
                    # imwrite(I_preds[i], f"I_preds_{i}.png", normalize=True)
