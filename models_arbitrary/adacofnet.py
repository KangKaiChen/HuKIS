...
class QuaternionEncoder(torch.nn.Module):
    ...
    def forward(self, quaternions):
        ...
        twovector_rect3 = torch.stack((2 * i, 2 * j, 2 * k), dim=-1)
        vector_norm = torch.sum(twovector_rect3**2, dim=-1, keepdim=True)
        vector_norm = torch.sqrt(vector_norm)
        vector_norm = torch.cat((vector_norm, vector_norm, vector_norm), dim=-1)
        vector_norm[vector_norm == 0] = 1

        twovector_rect1 = 2 * r
        twovector_rect2 = twovector_rect1 * twovector_rect3 / vector_norm

        rot_matrix = torch.cat(
            (
                1 - vector_norm + 2 * r**2 * torch.ones_like(vector_norm),
                twovector_rect2[..., 2:],
                -twovector_rect2[..., 1:2],
                -twovector_rect2[..., :1],
                1 - vector_norm + 2 * i**2 * torch.ones_like(vector_norm),
                twovector_rect2[..., 0:1],
                twovector_rect2[..., 1:2],
                -twovector_rect2[..., 2:],
                1 - vector_norm + 2 * j**2 * torch.ones_like(vector_norm),
            ),
            dim=-1,
        )
        rot_matrix = rot_matrix.view(batch_size, 3, 3)

        # 将旋转矩阵投影到特征图像上
        feat_map = rot_matrix.unsqueeze(1)  # (batch, 1, 3, 3)
        feat_map = self.fc(feat_map.view(batch_size, -1)).view(
            batch_size, -1, 8, 8
        )  # (batch, out_dim, 8, 8)
...
class SpatialFeatureNetwork1(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.layer0 = ResNet_Block(3, 8)
...
class AggregationWeightingNetwork(torch.nn.Module):
    ...
    #   self.layer8 = CustomNeuralNetwork()
    def forward(self, x):
        # x_8 = self.layer8(quat)
...
class AdaCoFNet(torch.nn.Module):
    def __init__(self, args):
        ...
        self.spatialFeatureNetwork1 = SpatialFeatureNetwork1()
        if args.decoder_with_gated_conv <= 0:
            self.refinementNetwork = RefinementNetwork(
                self.args.decoder_with_mask, self.args.concat_proxy
            )
        else:
            self.refinementNetwork = GatedConvRefinementNetwork(
                self.args.concat_proxy, self.args.no_pooling, self.args.single_decoder
            )
        if self.args.noDL_CNNAggregation > 0 or "CNN" in self.args.pooling_type:
            self.aggregationWeightingNetwork = AggregationWeightingNetwork(
                self.args.noDL_CNNAggregation, self.args.pooling_type == "CNN_flowError"
            )
    ...
    def forward(self, frames, F_kprime_to_k, F_n_to_k_s, F_k_to_n_s, disparity_image):
        ...
        if self.args.inference_with_frame_selection > 0:  # 沒進這裡
            import numpy as np
        ...
        #  angle_tensor = torch.tensor(degree, dtype=torch.float32)
        for i in range(len(frames)):
            features.append(self.spatialFeatureNetwork(frames[i]))
            disparity_features.append(disparity_image[i])

        #     print(features[i].shape,disparity_features[i].shape)
        # shift = int(degree[i] * 32 / 360)
        # 對特徵張量進行旋轉
        #   features.append(torch.roll(self.spatialFeatureNetwork(frames[i]), shifts=shift, dims=2))

        # 创建一个空列表，用于存放两两相加后的特征

        # for i, (features1, features2) in enumerate(zip(features, disparity_features)):
        #     # 将特征张量相加，并将结果覆盖原有的特征张量
        #     features[i] = features1 + features2
        ...
        tenWarpedFeatDisp = []
        tenWarpedMask = []
        for idx, (feat, disp) in enumerate(zip(features, disparity_features)):

            if self.args.all_backward > 0:
                ref_frame_flow = torch.nn.ReplicationPad2d((W, W, H, H))(
                    F_k_to_n_s[idx]
                )  # 關鍵真到相鄰真的光流
                ...
                #      print("ref_frame_flow",ref_frame_flow.shape)
                tenRef = torch.nn.ReplicationPad2d((W, W, H, H))(feat)
                #     tenDisp = torch.nn.ReplicationPad2d((W, W, H, H))(disp)
                #   print("tenRef",tenRef.shape)
                ...
                # tenWarpedFirstDisp = softsplat.FunctionSoftsplat(
                #     tenInput=tenDisp,
                #     tenFlow=ref_frame_flow,
                #     tenMetric=None,
                #     strType="average",
                ...
                # imwrite(feat, 'tenWarpedFirst.png', range=(0, 1))
            ...
            if self.args.bundle_forward_flow > 0:
                ...
                tenWarpedSecond = backwarp(
                    tenInput=tenWarpedFirst, tenFlow=F_kprime_to_k_pad
                )

                # tenWarpedSecondDisp = backwarp(
                #     tenInput=tenWarpedFirstDisp, tenFlow=F_kprime_to_k_pad
                # )
                #  print("ref_frame_flow",ref_frame_flow.shape)
                tenMaskSecond = backwarp(
                    tenInput=tenMaskFirst, tenFlow=F_kprime_to_k_pad
                )
            """back to original resolution"""
            if self.args.FOV_expansion <= 0:
                # 跑這裡
                tenWarped = tenWarpedSecond[:, :, H:-H, W:-W]
                # tenWarpedDisp = tenWarpedSecondDisp[:, :, H:-H, W:-W]
                tenMask = tenMaskSecond[:, :, H:-H, W:-W]

            #      print(tenWarpedDisp.shape)
            else:
                tenWarped = tenWarpedSecond
                # tenWarpedDisp = tenWarpedSecondDisp
                tenMask = tenMaskSecond

            tenWarpedFeat.append(tenWarped)
            # tenWarpedFeatDisp.append(tenWarpedDisp)
            tenWarpedMask.append(tenMask)

        #   print(tenWarpedFeat[0].shape)
        ...
        if self.args.pooling_with_mask <= 0:
            ...
            if self.args.pooling_with_center_bias <= 0:
                ...
                global_summed_feature = torch.stack(tmp_list, -1).sum(-1)  # 0,1 判斷
                global_summed_weights = torch.stack(tenWarpedMask, -1).sum(
                    -1
                )  # 0,1 判斷
                ...
                if self.args.pooling_type == "gaussian":
                    ...
                    global_average_pooled_feature = torch.sum(
                        color_tensor * weight_tensor, dim=0
                    ) / torch.clamp(torch.sum(weight_tensor, dim=0), min=1e-6)
                    ...
                    def score_max(x, dim, score):
                        ...
                        return torch.gather(
                            x, dim, score.max(dim)[1].unsqueeze(dim).repeat(tuple(_tmp))
                        ).select(dim, 0)
                    ...
                    if self.args.inference_with_frame_selection > 0:
                        ...
                        middle_idx = len(frames) // 2
                    for i in range(len(frames)):
                        if self.args.FOV_expansion > 0:  # 沒跑這裡
                            flow_errors[i] = torch.nn.ReplicationPad2d((W, W, H, H))(
                                flow_errors[i]
                            )

                        #
                        if (
                            disparity_features[i].shape[2:]
                            != tenWarpedFeat[i].shape[2:]
                        ):
                            # 计算需要填充的宽度和高度，使其与 tenWarpedFeat[i] 的高度和宽度相同
                            pad_width = (
                                max(
                                    0,
                                    tenWarpedFeat[i].shape[3]
                                    - disparity_features[i].shape[3],
                                )
                                // 2
                            )
                            pad_height = (
                            ...
                            pad_layer = torch.nn.ReplicationPad2d(
                                (pad_width, pad_width, pad_height, pad_height)
                            )
                            disparity_features[i] = pad_layer(disparity_features[i])
                        # print(
                        #     tenWarpedFeat[i].shape,
                        #     tenWarpedMask[i].shape,
                        #     flow_errors[i].shape,
                        #     disparity_features[i].shape,
                        # )
                        weight_tensor.append(
                            self.aggregationWeightingNetwork(
                                torch.cat(
                ...
                #  print("global_average_pooled_feature",global_average_pooled_feature.shape)
        ...
        if self.args.no_pooling > 0 and self.args.single_decoder <= 0:
            ...
            if self.args.decoder_with_mask <= 0:
                ...
                for i in range(len(frames)):
                    I_pred, C = self.refinementNetwork(
                        torch.cat([tenWarpedFeat[i], global_average_pooled_feature], 1)
                    )
                    I_preds.append(I_pred)
                    Cs.append(C)
            else:
        ...
        if self.args.center_residual_detail_transfer > 0:
            ...
            reconstructed_frame, _ = self.refinementNetwork(
                torch.cat([features[center_idx], features[center_idx], tenOnes], 1)
            )
            ...
            I_preds[center_idx] = (
                I_preds[center_idx] + warped_delta_frame * tenWarpedMask[center_idx]
            )
            ...
            if self.args.residual_detail_transfer > 0:
                ...
                for i in range(len(frames)):
                    ...
                    if self.args.all_backward > 0:
                        ...
                        tenWarpedFirst = backwarp(
                            tenInput=tenRef, tenFlow=ref_frame_flow
                        )
                    else:
                        # 跑這裡
                        """padding for forward warping"""
        ...
        if self.args.seamless > 0:
            ...
            npWarpedKey = np.transpose(
                np.clip(tenWarpedKey.cpu().numpy(), 0.0, 1.0)[0, ::-1], (1, 2, 0)
            )
            np_I_pred = np.transpose(
                np.clip(I_pred.cpu().numpy(), 0.0, 1.0)[0, ::-1], (1, 2, 0)
            )
            np_mask_A = np.clip(tenWarpedMask[middle_idx].cpu().numpy(), 0.0, 1.0)[
                0, 0, :, :
            ]

            np_mask_B = np.clip(
                1.0 - tenWarpedMask[middle_idx].cpu().numpy(), 0.0, 1.0
            )[0, 0, :, :]
            ...
            if np_mask_A.shape[0] % minimum_multi_band != 0:
                pad_h_multi_band = (
                    minimum_multi_band - (np_mask_A.shape[0] % minimum_multi_band)
                ) // 2
                pad_b_multi_band = (
                    minimum_multi_band
                    - (np_mask_A.shape[0] % minimum_multi_band)
                    - pad_h_multi_band
                )
                print(pad_h_multi_band)
                print(pad_b_multi_band)
                np_mask_A = cv2.copyMakeBorder(
                    np_mask_A,
                    pad_h_multi_band,
                    pad_b_multi_band,
                    0,
                ...
                npWarpedKey = cv2.copyMakeBorder(
                    npWarpedKey,
                    pad_h_multi_band,
                    pad_b_multi_band,
                    0,
            ...
            if np_mask_A.shape[1] % minimum_multi_band != 0:
                pad_l_multi_band = (
                    minimum_multi_band - (np_mask_A.shape[1] % minimum_multi_band)
                ) // 2
                pad_r_multi_band = (
                    minimum_multi_band
                    - (np_mask_A.shape[1] % minimum_multi_band)
                    - pad_l_multi_band
                )
                np_mask_A = cv2.copyMakeBorder(
                    np_mask_A,
            ...
            I_pred = torch.from_numpy(
                np.expand_dims(np.transpose(frame_A[:, :, ::-1], (2, 0, 1)), 0).astype(
                    np.float32
                )
            ).cuda()
