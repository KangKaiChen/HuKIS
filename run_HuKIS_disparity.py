...
def disparity_matrix(frame_dofs, input_frame, mymodel):
    one_quat = [torch.tensor(sublist[:4]) for sublist in frame_dofs]
    ...
    smooth_out = mymodel(one_quat_stacked)
    # print(smooth_out)
    # 計算四元數的差異
...
def calc_flow(img1, img2):
    with torch.no_grad():
        ...
        flow_low, flow_up = flow_model(
            images[0, None], images[1, None], iters=20, test_mode=True
        )
...
def read_homography(H_path):
    xv, yv = np.meshgrid(
        np.linspace(0, 832 + 2 * 64 - 1, 832 + 2 * 64),
        np.linspace(0, 448 + 2 * 64 - 1, 448 + 2 * 64),
    )
    ...
    if np.sum(np.abs(H_inv)) == 0.0:
        ...
        H_inv[1, 1] = 1.0
        H_inv[2, 2] = 1.0
    xv_prime = (H_inv[0, 0] * xv + H_inv[0, 1] * yv + H_inv[0, 2]) / (
        H_inv[2, 0] * xv + H_inv[2, 1] * yv + H_inv[2, 2]
    )
    yv_prime = (H_inv[1, 0] * xv + H_inv[1, 1] * yv + H_inv[1, 2]) / (
        H_inv[2, 0] * xv + H_inv[2, 1] * yv + H_inv[2, 2]
    )
...
def read_flo(flo_path):
    print(flo_path)
    xv, yv = np.meshgrid(
        np.linspace(-1, 1, 832 + 2 * 64), np.linspace(-1, 1, 448 + 2 * 64)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--load", type=str, default="FuSta_model/checkpoint/model_epoch050.pth"
    )
    ...
    num_heads = 4  # 注意力头的数量
    mymodel = dvsnet.TransformerModel(
    ...
    with torch.no_grad():
        ...
        all_imgs = sorted(
            glob.glob(os.path.join(INPUT_FRAMES_PATH, "*.png"))
        )  # all pngs in a sequence
        all_quaternions = pd.read_csv(
            os.path.join(INPUT_FRAMES_PATH, "frame_6dof.csv"), header=None
        )
        all_quaternions_np = all_quaternions.values

        frame_dof_list.extend(all_quaternions_np)
        ...
        all_imgs = (
            [first_frame] * (GAUSSIAN_FILTER_KSIZE // 2)
            + all_imgs
            + [last_frame] * (GAUSSIAN_FILTER_KSIZE // 2)
        )
        ...
        for idx in range(
            GAUSSIAN_FILTER_KSIZE // 2, (GAUSSIAN_FILTER_KSIZE // 2) + original_length
        ):
            ...
            tenSecond = torch.FloatTensor(
                np.ascontiguousarray(
                    cv2.imread(filename=keyframe, flags=-1)[..., ::-1]
            ...
            if int(img_name[:-4]) == 0:
                ...
                if os.path.isfile(
                    os.path.join(
                        CVPR2020_warping_field_path,
                        str(int(img_name[:-4]) - 1).zfill(5) + "_H_inv.npy",
                    )
                ) and os.path.isfile(
                    os.path.join(
                        CVPR2020_warping_field_path,
                        str(int(img_name[:-4]) - 1).zfill(5) + ".npy",
                    )
                ):
                    tenH_inv = torch.FloatTensor(
                        np.ascontiguousarray(
                            read_homography(
                                os.path.join(
                                    CVPR2020_warping_field_path,
            ...
            F_kprime_to_k = torch.stack(
                (totalFlow[:, 0] * W_ratio, totalFlow[:, 1] * H_ratio), dim=1
            )
            ...
            forward_flows = []
            backward_flows = []
            for frame_shift in range(
                -(GAUSSIAN_FILTER_KSIZE // 2),
                (GAUSSIAN_FILTER_KSIZE // 2) + 1,
                ...
                input_frames.append(
                    torch.FloatTensor(
                        np.ascontiguousarray(
                            cv2.imread(filename=ref_frame, flags=-1)[..., ::-1]
                            .transpose(2, 0, 1)[None, :, :, :]
                            .astype(np.float32)
                            * (1.0 / 255.0)
                        )
                ...
                match = re.search(r"(\d+)", ref_frame)
                if match:
                    index = int(match.group(1))
                    print(index, len(frame_dof_list))
                    frame_dofs.append(frame_dof_list[index - 1])
            ...
            input_frames = [
                x[
                    :,
                    :,
                    boundary_cropping_h:-boundary_cropping_h,
                    boundary_cropping_w:-boundary_cropping_w,
                ]
                for x in input_frames
            ]
            F_kprime_to_k = F_kprime_to_k[
                :,
                :,
                boundary_cropping_h:-boundary_cropping_h,
                boundary_cropping_w:-boundary_cropping_w,
            ...
            disparity_image = disparity_matrix(frame_dofs, input_frames, mymodel)
            print(disparity_image[0].shape)
            print("input_frames", input_frames[0].shape)
            frame_out = model(
                input_frames,
                F_kprime_to_k,
                forward_flows,
            ...
            if not os.path.exists("tmp/"):
                os.makedirs("tmp/")
            np.save(
                "tmp/" + str(len(large_mask_chain)).zfill(5),
                frame_out.detach().cpu().numpy(),
            )
            ...
            F_kprime_to_k_pad = torch.nn.ReplicationPad2d((WWW, WWW, HHH, HHH))(
                F_kprime_to_k
            )
            tenWarpedFeat = []
            tenWarpedMask = []
            for iii, feat in enumerate(input_frames):
                ...
                ref_frame_flow = torch.nn.ReplicationPad2d((WWW, WWW, HHH, HHH))(
                    forward_flows[iii]
                )
        ...
        for level in range(5, -1, -1):
            ...
            for iiii in range(len(large_mask_chain)):
                ...
                for uu in range(-maximum_movement, maximum_movement + 1):
                    ...
                    for vv in range(-maximum_movement, maximum_movement + 1):
                        # converage term
                        motion_vector_u = int(
                            accumulated_motion_vectors[iiii, 0] + uu * (2**level)
                        )
                        motion_vector_v = int(
                            accumulated_motion_vectors[iiii, 1] + vv * (2**level)
                        )
                        ...
                        cropped_mask = large_mask_chain[iiii][
                            :,
                            :,
                            HHH + motion_vector_u : -HHH + motion_vector_u,
                        ...
                        data_term[
                            iiii,
                            (uu + maximum_movement) * (2 * maximum_movement + 1)
                            + vv
                            + maximum_movement,
                        ] = summed_mask

            print("smoothness term")
            smoothness_term = np.zeros(
                (
                    (2 * maximum_movement + 1) * (2 * maximum_movement + 1),
                    (2 * maximum_movement + 1) * (2 * maximum_movement + 1),
                )
                    ...
                    for uuuu in range(-maximum_movement, maximum_movement + 1):
                        for vvvv in range(-maximum_movement, maximum_movement + 1):
                            smoothness_term[
                                ...
                                + vv
                                + maximum_movement,
                                (uuuu + maximum_movement) * (2 * maximum_movement + 1)
                                + vvvv
                                + maximum_movement,
                            ] = ((uu - uuuu) * (2**level)) ** 2 + (
                                (vv - vvvv) * (2**level)
            ...
            for iiii in range(labels.shape[0]):
                accumulated_motion_vectors[iiii, 0] += (
                    labels[iiii] // (2 * maximum_movement + 1) - maximum_movement
                ) * (2**level)
        ...
        for iiii in range(len(large_mask_chain)):
            ...
            # if OOM
            imwrite(
                torch.from_numpy(np.load(output_frames[iiii]))[
                    :,
                    :,
                    HHH + motion_vector_u : -HHH + motion_vector_u,
                    WWW + motion_vector_v : -WWW + motion_vector_v,
                ],
                os.path.join(OUTPUT_PATH, str(iiii + 1).zfill(5) + ".png"),
                range=(0, 1),
            )
